...
def val(args, val_loader, model, epoch):
    ...
    for iter, batched_inputs in enumerate(val_loader):
        ...
        img, target, mask_A, mask_B = batched_inputs
        rgb_pre_img = img[:, 0:3]
        rgb_post_img = img[:, 3:6]
        nir_pre_img = img[:, 6:9]
        nir_post_img = img[:, 9:12]

        start_time = time.time()

        if args.onGPU == True:
            rgb_pre_img = rgb_pre_img.cuda()
            nir_pre_img = nir_pre_img.cuda()
            target = target.cuda()
            rgb_post_img = rgb_post_img.cuda()
            nir_post_img = nir_post_img.cuda()
            # change_mask = change_mask.cuda()
            mask_A = mask_A.cuda()
            mask_B = mask_B.cuda()

        rgb_pre_img_var = torch.autograd.Variable(rgb_pre_img).float()
        rgb_post_img_var = torch.autograd.Variable(rgb_post_img).float()
        nir_pre_img_var = torch.autograd.Variable(nir_pre_img).float()
        nir_post_img_var = torch.autograd.Variable(nir_post_img).float()
        target_var = torch.autograd.Variable(target).float()

        output, output2, output3, output4 = model(rgb_pre_img_var, rgb_post_img_var, nir_pre_img_var, nir_post_img_var,
                                                  mask_A, mask_B)
        ...
        if np.mod(iter, 200) == 1:
            vis_input = utils.make_numpy_grid(utils.de_norm(rgb_pre_img_var[0:8]))
            vis_input2 = utils.make_numpy_grid(utils.de_norm(rgb_post_img_var[0:8]))
...
def trainValidateSegmentation(args):
    ...
    elif args.file_root == 'LSMD_1':
        args.file_root = r'E:\Github\LSMD'
    else:
    ...
    mean = [0.406, 0.456, 0.485, 0.406, 0.456, 0.485, 0.406, 0.456, 0.485, 0.406, 0.456, 0.485]
    std = [0.225, 0.224, 0.229, 0.225, 0.224, 0.229, 0.225, 0.224, 0.229, 0.225, 0.224, 0.229]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--file_root', default="LSMD_1", help='Data directory | LEVIR | BCDD | SYSU ')

...
class RandomExchange(object):
    def __call__(self, image, label):
        if random.random() < 0.5:
            ...
            nir_post = image[:, :, 9:12]
            # 交换 pre/post
            image = numpy.concatenate((rgb_post, rgb_pre, nir_post, nir_pre), axis=2)
        return [image, label]
...
class ToTensor(object):
    """
    Convert 12-channel image and label to torch tensors.
    - Channels order: [RGB_pre, RGB_post, NIR_pre, NIR_post]
    - Only convert RGB channels BGR->RGB
    """
    ...
    def __call__(self, image, label):
        ...
        # Convert RGB channels from BGR to RGB, keep NIR channels unchanged
        image[:, :, 0:3] = image[:, :, 0:3][:, :, ::-1]  # RGB_pre
        image[:, :, 3:6] = image[:, :, 3:6][:, :, ::-1]  # RGB_post
        # NIR_pre: 6:9, NIR_post: 9:12, do nothing

        # Transpose to [C,H,W]
        image = image.transpose((2, 0, 1))
        image_tensor = torch.from_numpy(image.astype(np.float32))

        # Convert label to tensor [1,H,W]
        label_tensor = torch.from_numpy(np.array(label, dtype=np.int64)).unsqueeze(0)
